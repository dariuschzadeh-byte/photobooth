...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$here = '%~dp0'.TrimEnd('\'); $desk = [Environment]::GetFolderPath('Desktop'); $sh = New-Object -ComObject WScript.Shell; $list = @(@('1 - START PHOTOBOOTH','START-BOOTH.bat'), @('2 - STOP PHOTOBOOTH','STOP-BOOTH.bat'), @('3 - UPDATE PHOTOBOOTH','UPDATE-BOOTH.bat'), @('4 - CHECK PHOTOS','CHECK-PHOTOS.bat'), @('5 - SHOW CODES','SHOW-CODES.bat'), @('6 - TRY BRIGHTNESS','TRY-EXPOSURE.bat')); foreach ($i in $list) { $t = Join-Path $here $i[1]; if (Test-Path $t) { $l = $sh.CreateShortcut((Join-Path $desk ($i[0] + '.lnk'))); $l.TargetPath = $t; $l.WorkingDirectory = $here; $l.Save(); Write-Host ('   ok   ' + $i[0]) } else { Write-Host ('   skip ' + $i[0] + '  (' + $i[1] + ' not found)') } }"
